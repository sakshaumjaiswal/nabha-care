...
import { 
  Video, Stethoscope, FileText, MapPin, Calendar, Clock, User, Check, Loader2, Phone
} from 'lucide-react';
...
const PatientDashboard: React.FC = () => {
  ...
  // CHANGED: The hook now provides real, live data.
  ...
  // CHANGED: Logic now directly uses the 'consultations' state from the hook.
  const upcomingAppointment = useMemo(() => 
    consultations.find(c => c.status === 'scheduled' || c.status === 'in-progress'), 
  [consultations]);
  ...
  // CHANGED: Simplified loading state to handle both auth and data fetching.
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container px-6 py-8 max-w-7xl">
        <div className="mb-8">
          {/* UPDATED: Displays the user's name from the profile. */}
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-8">
            ...
            <div>
              ...
              <div className="space-y-3">
                {/* UPDATED: This section now maps over live data. */}
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                           ...
                           <div>
                            {/* UPDATED: Fetches the doctor's name from the joined data. */}
                            <h3 className="font-medium">Dr. {consultation.doctors?.name || 'N/A'}</h3>
                        ...
                        <div className="flex items-center gap-2">
                          <span className="px-2 py-1 text-xs bg-medical-success/10 text-medical-success rounded-full capitalize">{consultation.status}</span>
                          <Button variant="ghost" size="sm" asChild><Link to={`/records?id=${consultation.id}`}>View Record</Link></Button>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
          ...
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent>
                {/* UPDATED: This section now shows the real upcoming appointment. */}
                {upcomingAppointment ? (
                  <div>
                     <div className="flex items-center gap-3 mb-3">
                         <User className="h-5 w-5 text-medical-primary"/>
                         <span className="font-medium">Dr. {upcomingAppointment.doctors?.name}</span>
                     </div>
                     <div className="flex items-center gap-3 text-sm text-muted-foreground mb-4">
                         <Calendar className="h-4 w-4"/>
                         <span>{new Date(upcomingAppointment.scheduled_at).toLocaleDateString()} at {new Date(upcomingAppointment.scheduled_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                     </div>
                     {/* REMOVED: "Mark as Complete" button is not a patient action. Replaced with "Join Call". */}
                     <Button size="sm" className="w-full" asChild>
                        <Link to={`/consult/call?room=${upcomingAppointment.room_id || upcomingAppointment.id}`}>
            ...
            <Card className="border-medical-error/30 bg-medical-error/5">
              ...
              <CardContent>
                ...
                <Button variant="destructive" className="w-full" asChild>
                  <a href="tel:102"><Phone className="h-4 w-4 mr-2" /> Call 102 (Ambulance)</a>
                </Button>
              </CardContent>
            </Card>
          </div>
...
const HealthInput = ({ id, label, type = "text", value, onChange }: { id: keyof HealthSummary; label: string; type?: string; value?: any; onChange: React.Dispatch<React.SetStateAction<HealthSummary>> }) => (
